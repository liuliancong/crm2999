...
*/
            ...
            //用户点击修改图标
            $("#customerRemark").on("click","a[name='editA']",function (){
                var customerRemarkId = $(this).attr("remarkId");
                // console.log(customerRemarkId)
                //获取备注内容
                var noteContent = $("#div_"+customerRemarkId+" h5").text();
                // console.log(noteContent)
                //弹出模态窗口
                $("#editRemarkModal").modal("show");
                //赋值
                $("#edit-remarkId").val(customerRemarkId);
            ...
            $("#updateRemarkBtn").click(function (){
                ...
                $.ajax({
                    url:"workbench/customer/saveEditCustomerRemarkByCustomerRemarkId.do",
                    data:{
                        id:id,
            ...
            //用户点击删除图标
            $("#customerRemark").on("click","a[name='deleteA']",function(){
                var customerRemarkId = $(this).attr("remarkId");
                // console.log(customerRemarkId)
                if(window.confirm("请确认删除！")){
                    $.ajax({
                        url:"workbench/customer/deleteCustomerRemarkByCustomerRemarkId.do",
                        data:{
                            customerRemarkId:customerRemarkId
                        },
                        dataType:"json",
                        type:"post",
                        success:function (data){
                            if(data.code==1){
                                $("#div_"+customerRemarkId).remove();
                            }else{
                                alert(data.message)
                            }
                        }
            ...
            $("#saveCustomerRemarkBtn").click(function (){
                ...
                $.ajax({
                    url:"workbench/customer/saveCreateCustomerRemarkByCustomerRemarkId.do",
                    data:{
                        noteContent:noteContent,
...
<!-- 备注 -->
<div style="position: relative; top: 10px; left: 40px;" id="customerRemark">
    <div class="page-header">
        <h4>备注</h4>
    </div>
    <c:forEach items="${customerRemarkList}" var="customerRemark">
        <div class="remarkDiv" style="height: 60px;" id="div_${customerRemark.id}">
            ...
            <div style="position: relative; top: -40px; left: 40px;" >
                ...
                <font color="gray">客户</font> <font color="gray">-</font> <b>${customer.name}</b> <small style="color: gray;"> ${customerRemark.editFlag == '0' ?customerRemark.createTime : customerRemark.editTime} 由${customerRemark.editFlag == '0' ?customerRemark.createBy : customerRemark.editBy}${customerRemark.editFlag == '0' ?"创建":"修改"}</small>
                <div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">
                    ...
                    &nbsp;&nbsp;&nbsp;&nbsp;
                    <a class="myHref" name="deleteA" remarkId="${customerRemark.id}" href="javascript:void(0);"><span class="glyphicon glyphicon-remove" style="font-size: 20px; color: #E6E6E6;"></span></a>
                </div>
            </div>
        </div>
    </c:forEach>
        ...
        <div style="position: relative;top: 0px;">
            <table id="activityTable2" class="table table-hover" style="width: 900px;">
                ...
                <c:forEach items="${tranList}" var="tran">
                    <tr>
                        <td><a href="/workbench/tran/detail.do?customerId=${customer.id}" style="text-decoration: none;">${tran.name}</a></td>
                        <td>${tran.money}</td>
                        <td>${tran.stage}</td>
                        <td>90</td>
                        <td>${tran.expectedDate}</td>
                        <td>${tran.type}</td>
                        <td><a href="javascript:void(0);" tranId="${tran.id}" data-toggle="modal" data-target="#removeTransactionModal" style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>删除</a></td>
                    </tr>
                </c:forEach>
<%--                <tr>
                    <td><a href="/workbench/tran/detail.do?customerId=${customer.id}" style="text-decoration: none;">动力节点-交易01</a></td>
                    <td>5,000</td>
                    <td>谈判/复审</td>
                    <td>90</td>
                    <td>2017-02-07</td>
        ...
        <div style="position: relative;top: 0px;">
            <table id="activityTable" class="table table-hover" style="width: 900px;">
                ...
                <c:forEach items="${contactsList}" var="contacts">
                    <tr>
                        <td><a href="/workbench/contacts/detail.do?contactsId=${contacts.id}" style="text-decoration: none;">${contacts.fullname}</a></td>
                        <td>${contacts.email}</td>
                        <td>${contacts.mphone}</td>
                        <td><a href="javascript:void(0);" contactsId="${contacts.id}" data-toggle="modal" data-target="#removeContactsModal" style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>删除</a></td>
                    </tr>
                </c:forEach>
<%--                <tr>
